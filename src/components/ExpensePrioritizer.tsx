...
import { PriorityBill, TransactionScope, TransactionType } from "../types";
import { 
  CheckCircle, 
  Clock, 
  ArrowRight, 
...
export default function ExpensePrioritizer({
  ...
  const [targetGroup, setTargetGroup] = useState<"G1" | "G2" | "G3">("G1");
    ...
    const updated = bills.map((b) => {
      if (b.id === id) {
        ...
        return {
          ...
          groupType: newStatus === "PAGAR" ? "G1" as const : "WAIT" as const,
        };
      }
      return b;
    });
  ...
  const handleChangeGroup = (id: string, group: "G1" | "G2" | "G3") => {
    const updated = bills.map((b) => {
      if (b.id === id) {
  ...
  const handlePayAndRegister = (bill: PriorityBill) => {
    ...
    const category = bill.scope === TransactionScope.PROFESSIONAL 
  ...
  const getSubgroupTotals = (group: "G1" | "G2" | "G3") => {
  ...
  const g1 = getSubgroupTotals("G1");
  const g2 = getSubgroupTotals("G2");
  const g3 = getSubgroupTotals("G3");
    ...
    <div id="expense-prioritizer" className="space-y-6">
      ...
      <div className="bg-slate-900 border-l-4 border-[#8b5cf6] p-4 text-white rounded-r-lg shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1 py-1 px-2.5 bg-slate-800 hover:bg-slate-700 transition font-bold text-[10px] uppercase tracking-wider rounded border border-slate-700 text-slate-350 cursor-pointer"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-[#e2e8f0] p-4 rounded-lg shadow-2xs">
          ...
          <p className="text-2xl font-bold font-mono text-amber-750">{formatCurrency(totalEsperar)}</p>
          <div className="mt-1 flex items-center gap-1.5 text-[10px] text-slate-500">
      ...
      <div className="bg-white border border-[#e2e8f0] p-4 rounded-lg shadow-2xs grid grid-cols-1 lg:grid-cols-12 gap-5 items-end">
        ...
        <form onSubmit={handleAddBill} className="lg:col-span-9 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
          <div className="md:col-span-2 space-y-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded p-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-hidden focus:border-[#8b5cf6]"
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded p-1.5 text-xs text-slate-800 focus:outline-hidden"
            >
          ...
          <button
            type="submit"
            className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold uppercase tracking-wider text-xs py-2 rounded transition flex items-center justify-center gap-1 cursor-pointer h-[34px]"
          >
        ...
        <div className="xl:col-span-8 space-y-6">
          <div className="bg-white border border-[#e2e8f0] rounded-lg shadow-2xs overflow-hidden">
            ...
            <div className="p-4 space-y-5">
              {/* SUBGROUP 1: R$ 3.971,09 */}
              <div className="border border-slate-100 rounded-lg overflow-hidden bg-slate-50/20">
                <div className="bg-slate-50 px-3.5 py-2 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-red-500 rounded-full"></span>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Subgrupo 1 • Custos Urgentes</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-slate-705">
                    {formatCurrency(g1.sum)} ({g1.count} itens)
                  </span>
                </div>
              ...
              {/* SUBGROUP 2: R$ 3.032,00 */}
              <div className="border border-slate-100 rounded-lg overflow-hidden bg-slate-50/20">
                <div className="bg-slate-50 px-3.5 py-2 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-orange-500 rounded-full"></span>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Subgrupo 2 • Acordos e Lazer</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-slate-705">
                    {formatCurrency(g2.sum)} ({g2.count} itens)
                  </span>
                </div>
              ...
              {/* SUBGROUP 3: R$ 1.516,00 */}
              <div className="border border-slate-100 rounded-lg overflow-hidden bg-slate-50/20">
                <div className="bg-slate-50 px-3.5 py-2 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-[#8b5cf6] rounded-full"></span>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Subgrupo 3 • Assinaturas e Planos</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-slate-705">
                    {formatCurrency(g3.sum)} ({g3.count} itens)
                  </span>
                </div>
          ...
          <div id="whatsapp-api-config-area" className="bg-slate-50 p-6 border-b border-slate-200">
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <form onSubmit={handleSaveSettings} className="grid grid-cols-1 md:grid-cols-2 gap-5 text-slate-800">
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 font-sans text-slate-805"
                      required
                    />
                ...
                <div className="space-y-4">
                  {integrationType === "webhook" ? (
                    <div className="bg-white card rounded-xl p-4 border border-slate-150 space-y-3 shadow-2xs">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs focus:ring-1 focus:ring-emerald-500 font-mono text-slate-805"
                        />
                      </div>
                    ...
                    <div className="bg-white card rounded-xl p-4 border border-slate-150 space-y-3 shadow-2xs">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-[10px] focus:ring-1 focus:ring-emerald-500 font-mono text-slate-805"
                          />
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs focus:ring-1 focus:ring-emerald-500 font-mono text-slate-850"
                        />
        ...
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-0 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 py-2 px-3 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-350 text-slate-600 hover:text-slate-800 transition rounded-lg text-xs font-bold shadow-2xs shrink-0 cursor-pointer"
            >
            ...
            <div className="lg:col-span-8 flex flex-col h-full bg-[#f0f2f5] relative">
              ...
              >
                <input
                  ...
                  className="flex-grow bg-white border border-slate-200 rounded-lg p-1.5 px-3 text-xs focus:ring-1 focus:ring-emerald-500 font-sans text-slate-850"
                />
                <button
                  type="submit"
